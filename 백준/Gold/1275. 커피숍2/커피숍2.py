...
class Main:
    def __init__(self):
        ...
        self.queries = list(tuple(map(lambda x: int(x)-1, input().split())) for _ in range(self.m))
    ...
    def solve(self):
        ...
        for x, y, a, b in self.queries:
            left = min(x, y)
            right = max(x, y)
            val = self.query(0, 0, self.n-1, left, right)
            self.answer.append(str(val))
            self.update(0, 0, self.n-1, a, b+1)
